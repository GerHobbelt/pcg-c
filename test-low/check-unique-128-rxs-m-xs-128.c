...
#define XX_INFO \
            "pcg_unique_128_rxs_m_xs_128_random_r:\n" \
            "      -  result:      128-bit unsigned int (pcg128_t)\n" \
            "      -  period:      2^128   (* #rngs  [unique stream per rng])\n" \
...
#define XX_SRANDOM(...)             \
            pcg_unique_128_srandom_r(&rng, __VA_ARGS__)
#define XX_RANDOM()                 \
            pcg_unique_128_rxs_m_xs_128_random_r(&rng)
#define XX_BOUNDEDRAND(bound)       \
            pcg_unique_128_rxs_m_xs_128_boundedrand_r(&rng, bound)
#define XX_ADVANCE(delta)           \
            pcg_unique_128_advance_r(&rng, delta)
